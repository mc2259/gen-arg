...
MAX_CONTEXT_LENGTH=700 # measured in words 
MAX_LENGTH=1024
MAX_TGT_LENGTH=200
    ...
    def create_gold_gen(self, ex, ontology_dict,mark_trigger=False, index=0, ent2info=None, use_info=False):
        ...
        for role in role2arg.keys():
            if role not in ontology_dict[evt_type]:
                print('Role:{} not found in ontology'.format(role))
                # annotation error 
                continue 
        ...
        if mark_trigger:
            ...
            tgt = self.tokenizer.tokenize(' '.join(context_words[trigger['start_idx']: trigger['end_idx']]), add_prefix_space=True)
            
            suffix = self.tokenizer.tokenize(' '.join(context_words[trigger['end_idx']:]), add_prefix_space=True)
